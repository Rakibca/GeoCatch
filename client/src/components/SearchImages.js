  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const imageData = items.map((image) => ({
        ...
        image: image.volumeInfo.imageLinks?.thumbnail || '',
      }));
            ...
            return (
              <Card key={image.imageId} border='dark'>
                ...
                <Card.Body>
                  ...
                  {Auth.loggedIn() && (
                    <Button
                      disabled={savedImageIds?.some((savedImageId) => savedImageId === image.imageId)}
                      className='btn-block btn-info'
                      onClick={() => handleSaveImage(image.imageId)}>
                      {savedImageIds?.some((savedImageId) => savedImageId === image.imageId)
